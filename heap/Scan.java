...
public class Scan implements GlobalConst {
	 ...
	 *
	 * @exception InvalidInfoSizeException Invalid tuple size
	 * @exception IOException              I/O errors
	 *
	 * @param hf A HeapFile object
	 */
	public Scan(Heapfile hf) throws InvalidInfoSizeException, IOException {
		init(hf);
	}
	 ...
	 */
	public Map getNext(MID mid) throws InvalidInfoSizeException, IOException {
	 ...
	 */
	public boolean position(MID mid) throws InvalidInfoSizeException, IOException {
		MID nxtmid = new MID();
		boolean bst;
	 ...
	 * @param hf A HeapFile object
	 */
	private void init(Heapfile hf) throws InvalidInfoSizeException, IOException {
		_hf = hf;
	...
	private boolean firstDataPage() throws InvalidInfoSizeException, IOException {
		...
		if (datapageRid != null) {
			...
			try {
				recinfo = dirpage.getRecordInfo(datapageRid);
			}
			...
			dpinfo = new DataPageInfo(recinfo);
			...
			if (nextDirPageId.pid != INVALID_PAGE) {
				...
				if (datapageRid != null) {
					...
					if (recinfo.getLength() != DataPageInfo.size)
						return false;
	 ...
	 */
	private boolean nextDataPage() throws InvalidInfoSizeException, IOException {
		DataPageInfo dpinfo;
	 ...
	 * (new) current record.
	 */
	private boolean mvNext(MID mid) throws InvalidInfoSizeException, IOException {
		MID nextmid;
		boolean status;
